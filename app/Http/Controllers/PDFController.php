...
class PDFController extends Controller {
    ...
    public function gerarPdf(Request $request) {
        if ($request->download == 'download') {
            ...
            switch ($request->rb) {
                case 'Solicitacoes':
                	  $pdf = $this->solicitacaoPdfStream();
                    return $pdf->stream('Solicitacoes');
                    break;
                case 'Usuarios':
...
//--------------Solicitacao-------------------------------------------------------   
     private function solicitacaoPdfStream() {
        $solicitacao = Solicitacao::orderBy('id')->get();
        $view = view('pdf.solicitacoes_pdf', ['solicitacao' => $solicitacao]);
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHtml($view);
        return $pdf;
    }

    private function solicitacaoPdfDown() {
        $solicitacao = Solicitacao::orderBy('id')->get();
        return PDF::loadView('pdf.solicitacoes_pdf', ['solicitacao' => $solicitacao]);
    }
...
//--------------Servidor-------------------------------------------------------
    private function servidorPdfStream() {
        ...
        $setores = Servidor::orderBy('nome')->get();
